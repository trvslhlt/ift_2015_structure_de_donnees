...
public class CountKmers {
	...
	public static void main(String[] args) {
		...
		if (args.length > 1)
			kmerMinSize = Integer.parseInt(args[1]); // optional argument
		if (args.length > 2)
			kmerMaxSize = Integer.parseInt(args[2]); // optional argument
		...
		for (int kmerSize = kmerMinSize; kmerSize <= kmerMaxSize; kmerSize++) { // for each kmer size
			...
			for (int pos = 0; pos <= sequence.length() - kmerSize; pos++) { // for each kmer start pos
				...
				if (count == null)
					count = 0; // if new kmer, initial count = 0
			...
			for (String kmer : freq.keySet()) { // iterate over all kmers
				...
				if (freq.get(kmer) == 1)
					uniqCount[kmerSize - kmerMinSize]++; // count unique kmers of this size
			}
			// report data for each kmer size
			System.out.println("Size = " + kmerSize + "; total of " + freq.size() + " distinct kmers; "
					+ uniqCount[kmerSize - kmerMinSize] + " unique kmers" + "; most frequent kmer is " + maxKmer
