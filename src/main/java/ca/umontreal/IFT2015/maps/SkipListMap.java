...
import ca.umontreal.IFT2015.adt.pqueues.DefaultComparator;
...
public class SkipListMap<K,V> extends AbstractSortedMap<K,V> {
    ...
    private int nbCompareCalls = 0;
    private boolean trace = false;

    // compare utilities
    public int getNbCompareCalls() { return nbCompareCalls; }
    public void resetNbCompareCalls() { this.nbCompareCalls = 0; }
	...
	if( this.trace ) System.out.println( s );
    }
	...
	//this.resetNbCompareCalls();
	//report( "skipSearch( " + key + " ); nbCompareCalls = " + this.nbCompareCalls );
	SkipNode<K,V> p = this.start;
	while( p.getBelow() != null ) {
	    p = p.getBelow();
	    while( compare( key, p.getNext().getKey() ) >= 0 ) {
		//report( "checked with " + p.getNext().getKey() );
		//this.nbCompareCalls++;
		p = p.getNext();
	    }
	    //report( "going down due to comparison with " + p.getNext().getKey() );
	    //this.nbCompareCalls++; // needs to compare one last time to exit the while loop
	}
	//report( "p = " + p.getKey() + "; after " + this.nbCompareCalls + " comparisons" );
	return p;
    }
	...
	do {
	    ...
	    //report( "insert " + key + " at level " + i + " (h=" + this.h + ")" );
	    exit = this.coinFlip.nextBoolean();
	    if( !exit ) {
	...
	//this.resetNbCompareCalls();
	//report( "put( " + key + ", " + value + " ); nbCompareCalls = " + this.nbCompareCalls );
	SkipNode<K,V> p = this.skipSearch( key );
	if( compare( key, p.getKey() ) == 0 ) { // match exists
	    //this.nbCompareCalls++;
	    //report( "return " + value + " after " + this.nbCompareCalls + " comparisons" );
	    return p.setValue( value );
	}
	this.skipInsert( key, value, p ); // otherwise add new entry
	//report( "return null ; after " + this.nbCompareCalls + " comparisons" );
