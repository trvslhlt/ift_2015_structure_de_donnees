...
public class TreeMap<K, V> extends AbstractSortedMap<K, V> {
	...
	// protected methods as shorthands to wrap operations on the underlying linked
	// binary tree.
	...
	// return the position in p's subtree having given key, or else the terminal
	// leaf
	...
	// remove and return the value of the entry with key, if it exists, null
	// otherwise
	@Override
	public V remove(K key) throws IllegalArgumentException {
		...
		if (isExternal(p)) { // key not found
			...
			System.out.println("remove( " + leaf.getElement() + " )");
			...
			System.out.println("sib: " + sib.getElement());
	...
	public Entry<K, V> floorEntry(K key) throws IllegalArgumentException {
		...
		Position<Entry<K, V>> p = treeSearch(root(), key);
		if (isInternal(p))
			return p.getElement(); // found exact match
		while (!isRoot(p)) {
			if (p == right(parent(p)))
	...
	// return the entry with the least key strictly greater than key, if any
	@Override
	...
	// return an iterable of entries with keys in range [fromKey, toKey) (toKey
	// excluded)
	@Override
